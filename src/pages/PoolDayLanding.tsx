    ...
    <div className="bg-mahana-light min-h-[90vh] animate-fade-in-up">
      ...
      <div className="relative h-96 w-full overflow-hidden">
        ...
        <div className="absolute bottom-10 left-0 right-0 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-3">
          ...
          <h1 className="text-3xl sm:text-5xl font-black text-white tracking-tight">Pool Day / Pasadía de Sol</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-10">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-sand-50 border-b border-sand-200 text-xs font-bold text-turquoise-950 uppercase tracking-wider">
                    ...
                    <th className="py-4 px-6">{language === 'es' ? 'Lunes a Domingo Normal' : 'Standard Monday to Sunday'}</th>
                    <th className="py-4 px-6">{language === 'es' ? 'Días Festivos / Feriados' : 'Festive Days / Holidays'}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-sand-100 font-medium text-mahana-dark/95">
                  <tr className="hover:bg-sand-50/20 transition-colors">
                    ...
                    <td className="py-4.5 px-6 font-extrabold text-turquoise-900">B/. 5.50</td>
                    <td className="py-4.5 px-6 font-extrabold text-turquoise-900">B/. 10.00</td>
                  </tr>
                  <tr className="hover:bg-sand-50/20 transition-colors">
                    <td className="py-4.5 px-6 font-bold text-turquoise-950">{language === 'es' ? 'Niños de 0 a 4 años' : 'Children 0 to 4 years'}</td>
          ...
          <div className="bg-rose-50/30 rounded-3xl border border-rose-200/60 p-8 space-y-4">
        ...
        <div className="bg-white rounded-[32px] border border-sand-200 p-8 shadow-premium h-fit space-y-6 lg:sticky lg:top-24">
          ...
          <div className="border-t border-sand-100 pt-6 space-y-4 text-xs font-bold text-mahana-dark/75">
            <div className="flex justify-between items-center">
              ...
              <span>{language === 'es' ? '3 Piscinas Incluidas' : '3 Pools Included'}</span>
            </div>
            <div className="flex justify-between items-center">
              <span>{language === 'es' ? 'Servicios del Lodge' : 'Lodge Services'}</span>
              <span>{language === 'es' ? 'Duchas y vestidores' : 'Showers and changing rooms'}</span>
            </div>
            <div className="flex justify-between items-center">
              <span>{language === 'es' ? 'Comidas & Bebidas' : 'Meals & Drinks'}</span>
              <span className="text-turquoise-800">{language === 'es' ? 'A la Carta / Promo Bar' : 'A la Carte / Promo Bar'}</span>
            </div>
            <div className="flex justify-between items-center">
              <span>{language === 'es' ? 'Mascotas' : 'Pets'}</span>
              <span>{language === 'es' ? 'Sí (con correa bajo control)' : 'Yes (must remain on leash)'}</span>
            </div>
          </div>
